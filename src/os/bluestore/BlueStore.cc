...
#ifdef HAVE_LIBZBD
static void get_zone_offset_object_key(uint32_t zone, uint64_t offset,
                                       ghobject_t oid, std::string *key) {
  key->clear();
...
static int get_key_zone_offset_object(const string &key, uint32_t *zone,
                                      uint64_t *offset, ghobject_t *oid) {
  const char *p = key.c_str();
  if (key.length() <
      sizeof(uint64_t) + sizeof(uint32_t) + ENCODED_KEY_PREFIX_LEN + 1)
    return -1;
  ...
  while (true) {
    ...
    if (p->second->offset < pos) {
      ...
      if (p->second->data.length()) {
        ...
        r._add_buffer(cache,
                      new Buffer(&r, p->second->state, p->second->seq, 0, right,
                                 p->second->flags),
                      0, p->second.get());
      }
...
BlueStore::Collection::Collection(BlueStore *store_, OnodeCacheShard *oc,
    ...
    : CollectionImpl(store_->cct, cid), store(store_), cache(bc), exists(true),
      onode_space(oc), commit_queue(nullptr) {}
...
int BlueStore::_write_bdev_label(CephContext *cct, const string &path,
                                 bluestore_bdev_label_t label) {
...
int BlueStore::_open_fm(KeyValueDB::Transaction t, bool read_only,
  ...
  if (t) {
    ...
    if (bdev->is_smr() && freelist_type != "zoned") {
      derr << "SMR device but freelist_type = " << freelist_type
           << " (not zoned)" << dendl;
      return -EINVAL;
    }
    if (!bdev->is_smr() && freelist_type == "zoned") {
    ...
    fm->create(bdev->get_size(), alloc_size, zone_size, first_sequential_zone,
               t);
...
int BlueStore::_create_alloc() {
  ...
  alloc = Allocator::create(cct, allocator_type, bdev->get_size(), alloc_size,
                            zone_size, first_sequential_zone, "block");
  if (!alloc) {
...
#ifdef HAVE_LIBZBD
  if (freelist_type == "zoned") {
    Allocator *a = Allocator::create(cct, cct->_conf->bluestore_allocator,
                                     bdev->get_conventional_region_size(),
                                     alloc_size, 0, 0, "zoned_block");
    if (!a) {
      lderr(cct) << __func__ << " failed to create "
                 << cct->_conf->bluestore_allocator << " allocator" << dendl;
      delete alloc;
      return -EINVAL;
    }
...
int BlueStore::_init_alloc(std::map<uint64_t, uint64_t> *zone_adjustments) {
  ...
  if (bdev->is_smr()) {
    ...
    for (unsigned i = first_sequential_zone; i < num_zones; ++i) {
      ...
      ceph_assert(wp[i] <=
                  (i + 1) * zone_size); // pos might be at start of next zone
      ...
      if (zones[i].write_pointer > p) {
        derr << __func__ << " zone 0x" << std::hex << i
             ...
             << " > device write pointer 0x" << p << std::dec
        ...
        dout(1) << __func__ << " zone 0x" << std::hex << i
                << " device write pointer 0x" << p << " > bluestore pointer 0x"
                << zones[i].write_pointer << ", advancing 0x" << delta
                << std::dec << dendl;
    ...
    // start with conventional zone "free" (bluefs may adjust this when it
    // starts up)
    auto reserved = _get_ondisk_reserved();
    // for now we require a conventional zone
    ceph_assert(bdev->get_conventional_region_size());
    ceph_assert(shared_alloc.a !=
                alloc); // zoned allocator doesn't use conventional region
    ...
    dout(1) << __func__ << " loaded zone pointers: " << std::hex
...
void BlueStore::_post_init_alloc(
    const std::map<uint64_t, uint64_t> &zone_adjustments) {
  int r = 0;
#ifdef HAVE_LIBZBD
  if (bdev->is_smr()) {
    ...
    dout(1) << __func__ << " adjusting freelist based on device write pointers"
            << dendl;
    auto f = dynamic_cast<ZonedFreelistManager *>(fm);
    ceph_assert(f);
...
int BlueStore::_minimal_open_bluefs(bool create) {
  ...
  r = bluefs->add_block_device(
      bluefs_layout.shared_bdev, bfn, false,
...
int BlueStore::_open_db_and_around(bool read_only, bool to_repair) {
  ...
  // SMR devices may require a freelist adjustment, but that can only happen
  // after the db is read-write. we'll stash pending changes here.
...
void BlueStore::_close_db() {
  ...
  if (do_destage && fm && fm->is_null_manager()) {
    ...
    if (ret != 0) {
      derr << __func__
           << "::NCB::store_allocator() failed (continue with "
...
int BlueStore::add_new_bluefs_device(int id, const string &dev_path) {
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    r = bluefs->add_block_device(
        BlueFS::BDEV_NEWDB, p, cct->_conf->bdev_enable_discard, SUPER_RESERVED);
    ceph_assert(r == 0);
...
void BlueStore::_fsck_repair_shared_blobs(
  ...
  auto foreach_shared_blob =
      [&](std::function<void(coll_t, ghobject_t, uint64_t,
  ...
  foreach_shared_blob([&](coll_t cid, ghobject_t oid, uint64_t sbid,
        ...
        if (p != (*ctx.zone_refs)[zone].end()) {
          if (first_offset < p->second) {
            dout(20) << " slightly wonky zone ref 0x" << std::hex << zone
                     << " offset 0x" << p->second << " but first offset is 0x"
                     << first_offset << "; this can happen due to clone_range"
                     << dendl;
          } else {
            dout(20) << " good zone ref 0x" << std::hex << zone << " offset 0x"
                     << p->second << " <= first offset 0x" << first_offset
                     << std::dec << dendl;
          }
          (*ctx.zone_refs)[zone].erase(p);
        } else {
          derr << "fsck error: " << oid << " references zone 0x" << std::hex
               << zone << " but there is no zone ref" << std::dec << dendl;
          // FIXME: add repair
          ++errors;
        }
      }
  ...
  struct FSCKWorkQueue : public ThreadPool::WorkQueue_ {
    ...
    FSCKWorkQueue(std::string n, size_t _batchCount, BlueStore *_store,
        ...
        : WorkQueue_(n, ceph::timespan::zero(), ceph::timespan::zero()),
          batchCount(_batchCount), store(_store), sb_info_lock(_sb_info_lock),
          sb_info(&_sb_info), sb_ref_counts(&_sb_ref_counts),
          repairer(_repairer) {
    ...
    void _void_process(void *item, TPHandle &handle) override {
      ...
      BlueStore::FSCK_ObjectCtx ctx(
          ...
          batch->expected_store_statfs, batch->expected_pool_statfs, repairer);
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair) {
  ...
  apply_for_bitset_range(
      0, std::max<uint64_t>(min_alloc_size, SUPER_RESERVED), alloc_size,
      used_blocks,
    ...
    for (unsigned i = first_sequential_zone; i < num_zones; ++i) {
      ...
      if (zones[i].write_pointer > p &&
        ...
        derr << "fsck error: zone 0x" << std::hex << i
             << " bluestore write pointer 0x" << zones[i].write_pointer
             << " > device write pointer 0x" << p << " (with only 0x"
    ...
    if (depth != FSCK_SHALLOW) {
      ...
      if (it) {
        for (it->lower_bound(string()); it->valid(); it->next()) {
          ...
          if (r < 0) {
            derr << "fsck error: invalid zone ref key "
                 << pretty_binary_string(key) << dendl;
            if (repair) {
          ...
          dout(30) << " zone ref 0x" << std::hex << zone << " offset 0x"
                   << offset << " -> " << std::dec << oid << dendl;
          if (zone_refs[zone].count(oid)) {
            derr << "fsck error: second zone ref in zone 0x" << std::hex << zone
                 << " offset 0x" << offset << std::dec << " for " << oid
                 << dendl;
            if (repair) {
              repairer.remove_key(db, PREFIX_ZONED_CL_INFO, key);
            }
      ...
      if (freelist_type == "zoned") {
        ...
        for (uint32_t zone = first_sequential_zone; zone < num_zones; ++zone) {
          ...
          dout(10) << "  marking zone 0x" << std::hex << zone
                   ...
                   << std::dec << dendl;
          apply_for_bitset_range(offset, length, alloc_size, used_blocks,
                                 [&](uint64_t pos, mempool_dynamic_bitset &bs) {
                                   if (bs.test(pos)) {
                                     derr << "fsck error: zone 0x" << std::hex
                                          << zone << " has used space at 0x"
                                          << pos * alloc_size
        ...
        for (uint32_t zone = first_sequential_zone; zone < num_zones;
             ++zone, zone_dead = 0) {
          while (pos != decltype(used_blocks)::npos &&
                 (pos * min_alloc_size) / zone_size == zone) {
            dout(40) << " zone 0x" << std::hex << zone << " dead 0x"
                     << (pos * min_alloc_size) << "~" << min_alloc_size
                     << std::dec << dendl;
            zone_dead += min_alloc_size;
            pos = used_blocks.find_next(pos);
          }
          dout(20) << " zone 0x" << std::hex << zone << " dead is 0x"
                   << zone_dead << std::dec << dendl;
          // cross-check dead bytes against zone state
          if (a->get_dead_bytes(zone) != zone_dead) {
            derr << "fsck error: zone 0x" << std::hex << zone << " has 0x"
                 << zone_dead << " dead bytes but freelist says 0x"
                 << a->get_dead_bytes(zone) << dendl;
            ++errors;
            // TODO: repair
          }
        }
        ...
        while (fm->enumerate_next(db, &offset, &length)) {
          ...
          if (intersects) {
            derr << "fsck error: free extent 0x" << std::hex << offset << "~"
                 << length << std::dec << " intersects allocated blocks"
                 << dendl;
            ++errors;
          }
        }
    ...
    if (per_pool_omap != OMAP_PER_POOL) {
      if (cct->_conf->bluestore_warn_on_no_per_pool_omap) {
        per_pool = "legacy (not per-pool) omap detected, "
                   "suggest to run store repair to benefit from per-pool omap "
                   "usage statistics";
      }
    }
  }
...
int BlueStore::_collection_list(Collection *c, const ghobject_t &start,
  ...
  auto log_latency = make_scope_guard(
      [&, start_time = mono_clock::now(), func_name = __func__] {
        log_latency_fn(func_name, l_bluestore_remove_lat,
...
int BlueStore::_open_super_meta() {
  ...
  // smr fields
  {
    ...
    if (r >= 0) {
      auto p = bl.cbegin();
      decode(zone_size, p);
      dout(1) << __func__ << " zone_size 0x" << std::hex << zone_size
              << std::dec << dendl;
...
void BlueStore::_txc_finalize_kv(TransContext *txc, KeyValueDB::Transaction t) {
  ...
  if (bdev->is_smr()) {
    for (auto &i : txc->old_zone_offset_refs) {
      ...
      string key;
      get_zone_offset_object_key(i.first.second, i.second, i.first.first->oid,
                                 &key);
...
void BlueStore::_zoned_cleaner_thread() {
  ...
  while (true) {
    ...
    if (zone_to_clean < 0) {
      ...
      auto period =
          ceph::make_timespan(cct->_conf->bluestore_cleaner_sleep_interval);
...
void BlueStore::_zoned_clean_zone(uint64_t zone, ZonedAllocator *a,
                                  ZonedFreelistManager *f) {
  dout(10) << __func__ << " cleaning zone 0x" << std::hex << zone << std::dec
           << dendl;
  ...
  for (it->lower_bound(zone_start); it->valid(); it->next()) {
    ...
    if (r < 0) {
      derr << __func__ << " failed to decode zone ref "
    ...
    dout(10) << __func__ << " zone 0x" << std::hex << zone << " offset 0x"
             << offset << std::dec << " " << oid << dendl;
    _clean_some(oid, zone);
  }

  if (a->get_live_bytes(zone) > 0) {
    derr << "zone 0x" << std::hex << zone << " still has 0x"
         << a->get_live_bytes(zone) << " live bytes" << std::dec << dendl;
    // should we do something else here to avoid a live-lock in the event of a
    // problem?
    return;
  }
  ...
  dout(10) << __func__ << " resetting zone 0x" << std::hex << zone << std::dec
           << dendl;
  ...
  dout(10) << __func__ << " done cleaning zone 0x" << std::hex << zone
           << std::dec << dendl;
  a->reset_zone(zone);
}

void BlueStore::_clean_some(ghobject_t oid, uint32_t zone) {
  dout(10) << __func__ << " " << oid << " from zone 0x" << std::hex << zone
           << std::dec << dendl;
...
int BlueStore::_do_alloc_write(TransContext *txc, CollectionRef coll,
  ...
  for (auto &wi : wctx->writes) {
    if (c && wi.blob_length > min_alloc_size) {
      ...
      int r = c->compress(wi.bl, t, compressor_message); // gotca
...
int BlueStore::_do_clone_range(TransContext *txc, CollectionRef &c,
  ...
  if (bdev->is_smr()) {
    ...
    for (auto e = newo->extent_map.extent_map.lower_bound(dummy);
         e != newo->extent_map.extent_map.end(); ++e) {
      ...
      for (auto &ex : e->blob->get_blob().get_extents()) {
        ...
        if (!newo->onode.zone_offset_refs.count(zone)) {
          ...
          dout(20) << __func__ << " add ref zone 0x" << std::hex << zone
                   << " offset 0x" << zoff << std::dec << " -> " << newo->oid
                   << dendl;
...
int BlueStore::_rename(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
  ...
  if (bdev->is_smr()) {
    ...
    for (auto &[zone, offset] : newo->onode.zone_offset_refs) {
      dout(20) << __func__ << " rm ref zone 0x" << std::hex << zone
               << " offset 0x" << offset << std::dec << " -> " << oldo->oid
               << dendl;
      string key;
      ...
      dout(20) << __func__ << " add ref zone 0x" << std::hex << zone
               << " offset 0x" << offset << std::dec << " -> " << newo->oid
               << dendl;
...
int load_bluefs_extents(BlueFS *bluefs, bluefs_layout_t *bluefs_layout,
...
Allocator *BlueStore::create_bitmap_allocator(uint64_t bdev_size) {
  ...
  uint64_t alloc_size = min_alloc_size;
  Allocator *alloc =
      Allocator::create(cct, "bitmap", bdev_size, alloc_size, zone_size,
...
int BlueStore::__restore_allocator(Allocator *allocator, uint64_t *num,
  ...
  while (extents_bytes_left) {
    ...
    read_bytes = bluefs->read(p_handle.get(), offset, sizeof(crc), nullptr,
                              (char *)&crc);
  ...
  // finally, read teh trailer and verify it is in good shape and that we got
  // all the extents
  {
...
int BlueStore::add_existing_bluefs_allocation(Allocator *allocator,
  ...
  if (bluefs) {
    ...
    for (auto itr = bluefs_extents.begin(); itr != bluefs_extents.end();
         extent_count++, itr++) {
...
Allocator *BlueStore::clone_allocator_without_bluefs(Allocator *src_allocator) {
  ...
  // during allocator-destage as they are stored elsewhere
  {
    ...
    load_bluefs_extents(bluefs, &bluefs_layout, cct, path, bluefs_extents_vec,
                        min_alloc_size);
    // then remove them from the shared allocator before dumping it to disk
    // (bluefs stored them internally)
    for (auto itr = bluefs_extents_vec.begin(); itr != bluefs_extents_vec.end();
         ++itr) {

...
static const uint64_t bit_mask_u64[65]
...
static inline void output_nbits_m64(uint64_t *&ocur, uint32_t &obleft,
                                    uint64_t v, const uint32_t n) {
  if (n == 64) {
    *ocur |= v << obleft;
    ocur += 1;
    if (obleft)
...
static inline void output_nbits(uint64_t *&ocur, uint32_t &obleft, uint64_t v,
                                const uint32_t n) {
  v &= (1ul << n) - 1;
  *ocur |= v << obleft;
  obleft += n;
...
static inline uint64_t input_nbits(uint64_t *&icur, uint32_t &ibleft,
                                   const uint32_t &n) {
...
uint32_t fits_kdecomp_f64(const uint8_t in[], const uint32_t iLen,
                          double out_[], uint32_t oLen, const uint32_t blk,
                          const uint32_t nsblk, const uint32_t inner) {
  ...
  for (; (uint8_t *)icur - in + (ibleft + 7) / 8 < iLen - 1; k += bigBlock) {
    for (uint32_t j = k; j < k + inner; j++) {
      ...
      if (sign_l) {
        for (uint32_t x = 0; x < lft; x++) {
          out[(i + x * inner) * 2 + 1] =
              ((0x00100000 & out[(i + x * inner) * 2 + 1]) << 11) +
              ((0xffe00000 & out[(i + x * inner) * 2 + 1]) >> 1) +
              (0x000fffff & out[(i + x * inner) * 2 + 1]);
        }
      }
      i += inner * lft;
      ...
      for (uint32_t _ = 0; _ < scc; _++) {
        ...
        if (bas_l == BW - 1) {
          ...
          if (input_nbits(icur, ibleft, 1)) {
            ...
            if (sign_l) {
              for (uint32_t x = 0; x < blk; x++) {
                out[(i + x * inner) * 2 + 1] = ((0x00100000 & avg_l) << 11) +
                                               ((0xffe00000 & avg_l) >> 1) +
                                               (0x000fffff & avg_l);
              }
            } else
        ...
        if (mx == mn) {
          ...
          if (sign_l)
            for (uint32_t x = 0; x < blk; x++) {
              out[(i + x * inner) * 2 + 1] = ((0x00100000 & rw[x]) << 11) +
                                             ((0xffe00000 & rw[x]) >> 1) +
                                             (0x000fffff & rw[x]);
...
uint32_t fits_kcomp_f64(
  ...
  } // Under maliciously crafted input, the compress bound may reach iLen*2 +
    // iLen/nsblk*(BW+LBW) + iLen/(16*blk)
  ...
  for (uint32_t k = 0; k < iLen; k += bigBlock) {
    for (uint32_t j = k; j < k + inner; j++) {
      ...
      if (sign_l > (blk >> 4)) {
        ...
        for (uint32_t x = 0; x < lft; x++) {
          rw[x] = ((0x7ff00000 & rw[x]) << 1) + ((0x80000000 & rw[x]) >> 11) +
                  ((0x000fffff & rw[x]));
        }
        sign_l = 1;
      } else {
      ...
      for (uint32_t _ = 0; _ < scc; _++) {
        ...
        if (bas_l == BW - 1) { // random
          ...
          if (sign_l) {
            for (uint32_t x = 0; x < blk; x++) {
              rw[x] = ((0x7ff00000 & rw[x]) << 1) +
                      ((0x80000000 & rw[x]) >> 11) + ((0x000fffff & rw[x]));
            }
          }
          sign_c >>= 31;
          if (sign_c > blk / 2)
...
uint32_t fits_kdecomp_f32(const uint8_t in[], const uint32_t iLen, float out_[],
                          uint32_t oLen, const uint32_t blk,
                          const uint32_t nsblk, const uint32_t inner) {
  uint32_t bigBlock = nsblk * inner;
  uint64_t *icur = (uint64_t *)in;
  uint32_t ibleft = 0;
  ...
  for (; (uint8_t *)icur - in + (ibleft + 7) / 8 < iLen - 1; k += bigBlock) {
    for (uint32_t j = k; j < k + inner; j++) {
      ...
      if (sign_l) {
        for (uint32_t x = 0; x < lft; x++) {
          out[i + x * inner] = ((0x00800000 & out[i + x * inner]) << 8) +
                               ((0xff000000 & out[i + x * inner]) >> 1) +
                               (0x007fffff & out[i + x * inner]);
        }
      }
      i += inner * lft;
      ...
      for (uint32_t _ = 0; _ < scc; _++) {
        ...
        if (bas_l == BW - 1) {
          ...
          if (input_nbits(icur, ibleft, 1)) {
            if (sign_l) {
              for (uint32_t x = 0; x < blk; x++) {
                out[i + x * inner] = ((0x00800000 & avg_l) << 8) +
                                     ((0xff000000 & avg_l) >> 1) +
                                     (0x007fffff & avg_l);
              }
            } else
        ...
        if (mx == mn) {
          ...
          if (sign_l)
            for (uint32_t x = 0; x < blk; x++) {
              out[i + x * inner] = ((0x00800000 & rw[x]) << 8) +
                                   ((0xff000000 & rw[x]) >> 1) +
                                   (0x007fffff & rw[x]);
  ...
  for (uint32_t k = 0; k < iLen; k += bigBlock) {
    for (uint32_t j = k; j < k + inner; j++) {
      ...
      if (sign_l > (blk >> 4)) {
        output_nbits(ocur, obleft, 1, 1);
        for (uint32_t x = 0; x < lft; x++) {
          rw[x] = ((0x7f800000 & rw[x]) << 1) + ((0x80000000 & rw[x]) >> 8) +
                  ((0x007fffff & rw[x]));
        }
        sign_l = 1;
      } else {
      ...
      for (uint32_t _ = 0; _ < scc; _++) {
        ...
        if (bas_l == BW - 1) { // random
          ...
          if (sign_l) {
            for (uint32_t x = 0; x < blk; x++) {
              rw[x] = ((0x7f800000 & rw[x]) << 1) +
                      ((0x80000000 & rw[x]) >> 8) + ((0x007fffff & rw[x]));
            }
          }
          sign_c >>= 31;
          if (sign_c > blk / 2)
...
uint32_t fits_kdecomp_u32(const uint8_t in[], const uint32_t iLen, dtype out[],
                          uint32_t oLen, const uint32_t blk,
                          const uint32_t nsblk, const uint32_t inner) {
  uint32_t bigBlock = nsblk * inner;
  uint64_t *icur = (uint64_t *)in;
...
uint32_t fits_kcomp_u32(
    ...
    const uint32_t blk,  /* parameter that controls the block size */
    const uint32_t
        nsblk,           /* length of dimension with lowest variability	*/
    const uint32_t inner /* the stride along this dimension */
) {
  uint32_t bigBlock = nsblk * inner;
  if (!bigBlock || iLen % bigBlock) {
...
uint32_t fits_kdecomp_u16(const uint8_t in[], const uint32_t iLen, dtype out[],
                          uint32_t oLen, const uint32_t blk,
                          const uint32_t nsblk, const uint32_t inner) {
  uint32_t bigBlock = nsblk * inner;
  uint64_t *icur = (uint64_t *)in;
...
uint32_t fits_kdecomp_u8(const uint8_t in[], const uint32_t iLen, dtype out[],
                         uint32_t oLen, const uint32_t blk,
                         const uint32_t nsblk, const uint32_t inner) {
  uint32_t bigBlock = nsblk * inner;
  uint64_t *icur = (uint64_t *)in;
...
uint32_t
fits_kcomp_u8(const dtype in[],    /* input array */
              const uint32_t iLen, /* input length */
              uint8_t out[],       /* output buffer */
              uint32_t oLen,       /* max length of output */
              const uint32_t blk,  /* parameter that controls the block size */
